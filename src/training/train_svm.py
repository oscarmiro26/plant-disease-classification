...
def train_and_evaluate():
    ...
    """
    logger, RUN_DIR = setup_logger(model_name="svm", base_log_dir=config.LOG_DIR)
    ...
    logger.info(
        "Loading and preparing training data for SVM using feature extraction: %s",
        feature_extraction_fn.__name__,
    )
    X_train, y_train = load_and_prepare_data(
        train_ds, feature_extraction_fn, n_jobs=N_JOBS
    )
    logger.info(
    ...
    X_test, y_test = load_and_prepare_data(
        test_ds, feature_extraction_fn, n_jobs=N_JOBS
    )
    ...
    pipeline = Pipeline(
        [
            ...
            ("svc", SVC(class_weight="balanced", cache_size=2000, kernel="rbf")),
        ]
    )
    ...
    target_names = [
        config.INV_LABEL_MAP[i]
        for i in sorted(config.INV_LABEL_MAP.keys())
