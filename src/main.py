...
@app.post("/predict/", response_model=PredictionResponse, tags=["Classification"], summary="Predict Plant Disease")
...
from fastapi.responses import JSONResponse
from fastapi.exceptions import RequestValidationError
from pydantic import BaseModel, field_validator, Field, ValidationError # Removed 'validator' as it's not used
from PIL import Image
import numpy as np
...
async def load_model():
    ...
    try:
        ...
        if os.path.exists(svm_path):
            ...
            print(f"Warning: SVM model file not found at {svm_path}. SVM predictions will not be available.")
    ...
    try:
        ...
        if os.path.exists(resnet_model_path):
            ...
            print(f"Warning: ResNet model file not found at {resnet_model_path}. ResNet predictions will not be available.")
...
@app.post(
    ...
    description=(
        "Please, choose a model, either SVM or ResNet50, and upload an image.\n"
        ...
        "The API will return a prediction for the chosen model and the uploaded image."
    ),
    responses={
        200: {
...
async def predict(input_data: ModelInput = Depends()):
    ...
    if input_data.model_type == "svm" and svm_model is None:
        raise HTTPException(status_code=500, detail="SVM model not loaded. Check server startup logs.")
    if input_data.model_type == "resnet" and resnet_model is None:
        raise HTTPException(status_code=500, detail="ResNet model not loaded. Check server startup logs.")
    ...
    if model is None:
        ...
        raise HTTPException(status_code=500, detail=f"Selected model '{input_data.model_type}' is not loaded.")

    try:
        contents = await input_data.file.read()
        # Ensure file is not empty after reading
        if not contents:
            raise HTTPException(status_code=400, detail="Uploaded file is empty. Please upload a valid image file.")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error during prediction for {input_data.model_type} model: {e}")
