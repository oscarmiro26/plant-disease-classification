...
def test_segmentation():
    ...
    try:
        ...
        print(
            f"Segmented mask (from RGB) shape: {segmented_image.shape}, dtype: {segmented_image.dtype}"
        )
        print(
            f"Unique values in segmented mask (from RGB): {np.unique(segmented_image)}"
        )
