...
logger, RUN_DIR = setup_logger(model_name="resnet152", base_log_dir=config.LOG_DIR)
...
def train_and_evaluate():
    ...
    class_weights = calculate_class_weights(train_df, config.LABEL_MAP).to(
        config.DEVICE
    )
    train_sampler = create_sampler(
        train_df, config.LABEL_MAP, use_mixing_sampler=True, alpha=MIX_ALPHA
    )
    ...
    samples_per_class = [
        train_df["label"].tolist().count(lbl) for lbl in config.LABEL_MAP
    ]
    criterion = Loss("focal_loss", samples_per_class, class_balanced=False).to(
        config.DEVICE
    )

    optimizer = torch.optim.AdamW(
        [{"params": model.fc.parameters(), "lr": LR_CLASSIFIER}], weight_decay=1e-4
    )
    ...
    torch.save(model.state_dict(), os.path.join(RUN_DIR, "resnet152.pth"))
    plt.figure()
