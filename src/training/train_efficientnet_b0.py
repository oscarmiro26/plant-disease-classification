...
NUM_WORKERS     = 4
LR_CLASSIFIER   = 1e-3
LR_LAYER4       = 1e-4    # EfficientNet has no .layer3/.layer4, but we keep these for compatibility if you wish to fine-tune blocks manually
LR_LAYER3       = 1e-5
UNFREEZE_L4_AT  = 5
UNFREEZE_L3_AT  = 10
STEP_SIZE       = 7
...
def train_and_evaluate():
    ...
    for epoch in range(1, NUM_EPOCHS+1):
        # (You can unfreeze blocks manually here if desired, e.g. model.features[6:], etc.)
        model.train()
        running_loss = 0.0
    ...
    torch.save(model.state_dict(), os.path.join(RUN_DIR, "efficientnet_b0.pth"))
    plt.figure()
