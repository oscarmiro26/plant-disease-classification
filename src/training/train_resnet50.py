...
#  Logging setup
logger, RUN_DIR = setup_logger(model_name="resnet50", base_log_dir=config.LOG_DIR)
...
def train_and_evaluate():
    ...
    # Balanced focal loss
    samples_per_class = [
        train_df["label"].tolist().count(lbl) for lbl in config.LABEL_MAP.keys()
    ]
    criterion = Loss(
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        scheduler.step()
        logger.info(
            f"Epoch {epoch:02d} | Train Loss: {epoch_train_loss:.4f} | Val Loss: {epoch_val_loss:.4f}"
        )
